...
**解决**：在 `backend/app/core/rag_chain.py` 的 `PLAIN_CHAT_SYSTEM_PROMPT_TEMPLATE` 中注入 `当前时间：{current_time}`

### 如何增加联网能力，处理天气等实时信息

至于天气等实时信息，模型本身无法获取，需要借助联网搜索工具。

**解决**：使用智谱的 `web_search` 工具联网搜索，通过 httpx 直接调用智谱原生 API（`backend/app/core/rag_chain.py` 的 `_web_search_chat_stream` 函数），前端提供联网搜索开关
...
**现象**：系统注入了当前日期，但星期名显示错误（如实际是星期三却显示星期四）

**原因**：Python `strftime("%A")` 输出英文星期名（如 "Wednesday"），模型翻译时出错

**解决**：在 `backend/app/core/rag_chain.py` 中改为手动中文星期名数组映射，避免模型翻译
...
- **方案B**：不传历史对话给联网搜索请求 → 部分改善但仍不稳定
- **方案C（最终方案）**：增加 `search_engine: "search_pro"` 和 `search_result: true` 参数 → 稳定 ✅
